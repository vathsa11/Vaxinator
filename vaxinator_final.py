...
class Hashing:
    ...
    # to avoid collission
    def Linear_Probing(self, curr_idx, records, key_probe):
        new_index = None
        ...
        if new_index is None:
            messagebox.showerror('ERROR!', 'File is Full. Cannot Insert any Record\nPlease Delete Unwanted Records to make space')
            screen1.destroy()
    ...
    def Insert(self, key, values):
        ...
        if result:
            ...
            if cell_chk is not None:
                messagebox.showwarning('COLLISSION!', 'Collission has occurred at position {} \nIt will be resolved by Linear Probing'.format(self.hash_value))
                screen1.destroy()
                self.Linear_Probing(self.hash_value, values, key)
    ...
    def Display(self):
        ...
        for row_idx,tup in enumerate(sheet.iter_rows(max_row=sheet.max_row, max_col=sheet.max_column, values_only=True)):

            if tup[0] is not None:
                row_num=row_idx+1
                Label(second_frame, bg='white', fg='blue', font=('tahoma',10), text="POSITION OF RECORD: {}".format(row_num)).pack()
                for data in tup:
...
# choice 1: Insert
    def getvalues():
                ...
                stat.pack()
                Label(text="").pack()
                Button(screen1, bg='#ebf6f9', fg='darkgreen', activebackground='green', activeforeground='white', text="SUBMIT", width=10, height=1, command=h.insert_values).pack()
...
# choice 2: Search
    def look():
                ...
                Label(screen2, height=2, width=50, font=('tahoma',10), text='Enter the Aadhaar Number of Record to be Searched: ').pack()
                adhrsrch = Entry(screen2, text=adhrsrch, relief='solid')
                adhrsrch.pack()
                Button(screen2, bg='#ebf6f9', fg='blue', activebackground='blue', activeforeground='white', text="Search", width=10, height=1, command=h.search_values).pack()
...
# choice 4: Update
    def change():
                ...
                Label(screen5, height=2, width=50, font=('tahoma',10), text='Enter the Aadhaar Number of Record to be Updated: ').pack()
                adhrupd = Entry(screen5, text=adhrupd, relief='solid')
                adhrupd.pack()
                Button(screen5, bg='#ebf6f9', fg='orangered', activebackground='lightyellow', activeforeground='orangered', text="OK", width=10, height=1, command=h.update_values).pack()
    ...
    def update_values(self):
                ...
                if fnd:
                    ...
                    if date2 == "na":
                        ...
                        if diff >= max_days:
                            Label(screen6, fg='green', font=('tahoma',12), text="\nThe person is ELIGIBLE for second dose").pack()
                            ...
                            statusupd.pack()
                            Button(screen6, bg='#ebf6f9', fg='orangered', activebackground='yellow', activeforeground='orangered', text='UPDATE', width=10, height=1, command=lambda: rec.Update(fnd)).pack()

                        else:
                            Label(screen6, fg='red', font=('tahoma',12), text="\nThe person is NOT eligible for decond dose now").pack()
                            visit = date1 + timedelta(days=max_days)
                            Label(screen6, font=('tahoma',10), text="\nEligible for second dose from:").pack()
...
# choice 5: Delete
    def delet():
                ...
                Label(screen7, height=2, width=50, font=('tahoma',10), text='Enter the Aadhaar Number of Record to be Deleted: ').pack()
                adhrdel = Entry(screen7, text=adhrdel, relief='solid')
                adhrdel.pack()
                Button(screen7, bg='#ebf6f9', fg='red', activebackground='red', activeforeground='white', text="OK", width=10, height=1, command=h.delete_values).pack()
...
# delete record from choice 5: Delete
    def delete_values(self):
                ...
                if fnd:
                    ...
                    Label(screen8, text="\n").pack()
                    Button(screen8, bg='#ebf6f9', font=('tahoma',8), fg='red', activebackground='red', activeforeground='white', text='DELETE', width=10, height=1, command=lambda: rec.Remove(fnd)).pack()
...
header=['Full Name: ','Aadhaar Number: ','DOB: ','Mobile number: ','Vaccine Name: ','Dose1 date: ','Dose2 date: ','Status: ']

h = Hashing

Label(text="").pack()                   # A widget used to display text (or image)on the screen.
Label(bg='lightgreen', font=('tahoma',20,'bold','underline'), height=2, width=23, text='Welcome to VAXINATOR!!').pack()

Label(text="").pack()                   # pack() adds widget to the screen
Label(text="").pack()

image = Image.open("syringe.png")               # opens the image
resize_image = image.resize((200, 200))          # resizing the image
img = ImageTk.PhotoImage(resize_image)            # adding resized image to the widget using PhotoImage class
label1 = Label(image=img)
label1.image = img
label1.pack()

Label(height=5, width=30, font=('tahoma',10), text='Get Vaccinated Now!').pack()
Label(height=2, width=23, font=('tahoma',14), text='Select an Option:').pack()

#flat, groove, raised, ridge, solid, or sunken

Button(height=2, width=20, bg='#ebf6f9', fg='darkgreen', activebackground='#b5fcb5', activeforeground='#023102', font=('tahoma',10), text='Insert Records', command=h.getvalues).pack()
Label(text="").pack()
Button(height=2, width=20, bg='#ebf6f9', fg='blue', activebackground='#ccccff', activeforeground='blue', font=('tahoma',10), text='Search Records', command=h.look).pack()
Label(text="").pack()
Button(height=2, width=20, bg='#ebf6f9', fg='black', activebackground='whitesmoke', activeforeground='black', font=('tahoma',10), text='Display Records', command=h.view).pack()
Label(text="").pack()
Button(height=2, width=20, bg='#ebf6f9', fg='orangered', activebackground='#ffffa6', activeforeground='orangered', font=('tahoma',10), text='Update Records', command=h.change).pack()
Label(text="").pack()
Button(height=2, width=20, bg='#ebf6f9', fg='red', activebackground='#ffcccc', activeforeground='red', font=('tahoma',10), text='Delete Records', command=h.delet).pack()
